...
import { assembleFinalVideo, generateSubtitlesFromScript, actualQualityFor } from '@/lib/shotstack'
...
// supportés par toute la chaîne (LTX, WAN, Pexels, Shotstack).
const productionSchema = z.object({
  step: z.enum(['script', 'video', 'voice', 'music', 'assembly', 'thumbnail', 'all']),
  idea: z.string().min(1, 'idee requise'),
...
export async function POST(req: Request) {
  try {
    ...
    const { step, idea, template, format, voice, musicStyle, tone } = parsed.data
    ...
    if (step === 'assembly') {
      ...
      const requestedQuality = profile.preferred_quality ?? '1080p'
    ...
    if (step === 'all') {
      ...
      const voiceId = voice ?? 'EXAVITQu4vr4xnSDxMaL'
      // Qualité = préférence du profil (plus de 1080p codé en dur qui ignorait
      // une préférence 4k/720p).
      const requestedQuality = profile.preferred_quality ?? '1080p'
      ...
      const [voiceBuffer, musicUrl, sceneResults] = await Promise.all([
        ...
        Promise.all(
          script.scenes
            .filter((s) => !s.use_stock)
            .map((scene) =>
              generateVisualWithFallback(scene.visual_prompt, requestedQuality, user.email, userPlan as Plan, format ?? '16:9')
            )
        ),
      ])
      ...
      const clips = sceneResults.map((r) => ({ url: r.url, type: r.source === 'pexels-stock' ? ('stock' as const) : ('ia' as const) }))
